...
const mockFetch = vi.fn().mockImplementation(() => Promise.resolve({ ok: true }));
...
describe("api: fetcher", () => {
  ...
  it("throws an error", async () => {
    ...
    await expect(fetcher(["/dogs/breeds", undefined, undefined])).rejects.toThrow(
      "Failed to fetch https://frontend-take-home-service.fetch.com/dogs/breeds",
    );
  });

  it("redirects on 401", async () => {
    mockFetch.mockImplementationOnce(() => Promise.resolve({ ok: false, status: 401 }));
