...
export const DogDetail: FC<Props> = ({ age, breed, className, img, name, zipCode }) => {
  return (
    <div
      className={clsx(
    ...
    >
      ...
      <div className="flex flex-col md:flex-1 md:max-w-lg">
        ...
        <p className="mx-6 text-lg italic">
          Hi, my name is <strong>{name}</strong>! I&apos;m a loving {breed} and am {age} year(s)
          old. The zip code I live in is {zipCode}. I can&apos;t wait to meet you!
        </p>
        <BiSolidQuoteAltRight className="size-6 self-end relative -top-4" />
        <Button className="mt-10" kind="link" size="large" href="/dashboard" replace>
          Find another match
        </Button>
      </div>
    </div>
