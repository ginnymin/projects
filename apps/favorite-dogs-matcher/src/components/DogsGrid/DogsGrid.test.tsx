...
describe("Components: DogsGrid", () => {
  it("renders grid, filters and pagination", () => {
    ...
    expect(screen.getByRole("button", { name: "Sort ascending by breed" })).toBeEnabled();
    expect(screen.getByRole("combobox", { name: "Filter by breed" })).toBeEnabled();
  ...
  it("renders loading state", () => {
    ...
    expect(screen.getByRole("button", { name: "Sort ascending by breed" })).toBeDisabled();
    expect(screen.getByRole("combobox", { name: "Filter by breed" })).toBeDisabled();
  });
  ...
  it("toggles sort", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Sort ascending by breed" }));

    expect(screen.getByRole("button", { name: "Sort descending by breed" })).toBeEnabled();

    await userEvent.click(screen.getByRole("button", { name: "Sort descending by breed" }));
  ...
  it("selects dogs and shows selected favorites button", async () => {
    ...
    expect(screen.getAllByRole("button", { name: "De-select as a favorite" })).toHaveLength(2);
  ...
  it("de-selects dogs", async () => {
    ...
    expect(screen.getByRole("button", { name: "De-select as a favorite" })).toBeVisible();

    await userEvent.click(screen.getByText("Luna"));

    expect(screen.queryByRole("button", { name: "De-select as a favorite" })).toBeNull();
  });
  ...
  it("de-selects dogs via the favorites dialog", async () => {
    ...
    expect(screen.queryByRole("button", { name: /View your favorits/ })).toBeNull();
    expect(screen.queryByRole("button", { name: "De-select as a favorite" })).toBeNull();
  });
  ...
  it("selects and removes breeds", async () => {
    ...
    await userEvent.click(screen.getByRole("option", { name: "Border Collie" }));
