...
describe("Components: BreedSelector", () => {
  it("should render combobox without options", () => {
    ...
    expect(screen.getByRole("combobox", { name: "Filter by breed" })).toBeVisible();
  ...
  it("should show options when focused", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByRole("option", { selected: false })).toHaveLength(4);
    });

    expect(screen.getByRole("option", { name: "Australian Shepherd" })).toBeVisible();
  ...
  it("should show selected options", async () => {
    render(<BreedSelector selectedBreeds={["Border Collie", "Corgi"]} onChange={mockOnChange} />);

    await userEvent.click(screen.getByRole("combobox"));

    await waitFor(() => {
      expect(screen.getByRole("option", { name: "Border Collie", selected: true })).toBeVisible();
      expect(screen.getByRole("option", { name: "Corgi", selected: true })).toBeVisible();
    });
  ...
  it("calls onChange when selecting an option", async () => {
    render(<BreedSelector selectedBreeds={["Corgi"]} onChange={mockOnChange} />);
    ...
    await userEvent.click(screen.getByRole("option", { name: "Australian Shepherd" }));
