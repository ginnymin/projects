...
describe("Components: RecipeForm", () => {
  ...
  it("renders", () => {
    ...
    expect(
      screen.getByText("No ingredients added yet! Add your first ingredient above."),
    ).toBeVisible();
  });
  ...
  it("renders with pre-filled data", () => {
    ...
    expect(screen.getByRole("textbox", { name: "Name" })).toHaveValue("My recipe");
    expect(screen.getAllByRole("spinbutton", { name: "Quantity" })).toHaveLength(3);
    expect(screen.getAllByRole("combobox", { name: "Unit" })).toHaveLength(3);
    expect(screen.getAllByRole("textbox", { name: "Ingredient" })).toHaveLength(3);
    expect(screen.getAllByRole("textbox", { name: "Ingredient" })[1]).toHaveValue("almond flour");
    expect(screen.getAllByRole("textbox", { name: "Ingredient" })[2]).toHaveValue("water");
  });

  it("enables add ingredient button", async () => {
    render(<RecipeForm onSave={mockOnSave} />);

    await userEvent.type(screen.getByRole("spinbutton", { name: "Quantity" }), "1");
    expect(screen.getByRole("button", { name: "Add" })).toBeDisabled();
    await userEvent.type(screen.getByRole("textbox", { name: "Ingredient" }), "honey");
  ...
  it("adds ingredient", async () => {
    ...
    expect(screen.getAllByRole("spinbutton", { name: "Quantity" })).toHaveLength(2);
    expect(screen.getAllByRole("combobox", { name: "Unit" })).toHaveLength(2);
    expect(screen.getAllByRole("textbox", { name: "Ingredient" })).toHaveLength(2);
  ...
  it("enables save button", async () => {
    ...
    await userEvent.type(screen.getByRole("textbox", { name: "Name" }), "smoothie");
