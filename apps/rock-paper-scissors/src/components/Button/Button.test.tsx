...
describe("Components: Button", () => {
  ...
  it("renders a link", () => {
    ...
    expect(screen.getByRole("link", { name: "Click" })).toHaveAttribute("href", "/link");
  });
  ...
  it("renders secondary button by default", () => {
    ...
    expect(screen.getByRole("button")).not.toHaveClass("bg-white text-primary-dark");
  });

  it("renders primary button", () => {
    render(<Button variant="primary">Click</Button>);

    expect(screen.getByRole("button")).toHaveClass("bg-white text-primary-dark");
    expect(screen.getByRole("button")).not.toHaveClass("border-2 border-white/50 ");
  });
